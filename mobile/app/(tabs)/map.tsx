...
import { Text, View, StyleSheet, ActivityIndicator, Modal, Image, Platform } from "react-native";
		...
		<View style={styles.container}>
			{errorMsg ? (
				...
				<>
					...
					>
						<YourLocationMarker lat={location.coords.latitude} long={location.coords.longitude} />
						...
						{data.map((field: Field) => (
							<Marker
								...
								title={field.name}
								onPress={Platform.OS === "android" ? () => handleMarkerPress(field) : undefined}
							>
								<CustomMarkerView
									field={field}
	...
	return (
		...
		>
			<View style={{ alignItems: "center" }}>
				<Image source={require("@/assets/images/person.png")} style={{ width: 30, height: 30 }} />
				{Platform.OS !== "android" && <Text style={styles.youMarker}>Tu ubicación</Text>}
			</View>
		</Marker>
	);
}

function CustomMarkerView({ field, onPress, styles }: { field: Field; onPress: () => void; styles: any }) {
...
function FieldModal({ field, visible, onClose }: { field: Field; visible: boolean; onClose: () => void }) {
	return (
		<Modal style={styles.modal} visible={visible} transparent={true} onRequestClose={onClose}>
