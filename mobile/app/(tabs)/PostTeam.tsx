...
export default function PostTeam({ navigation }: { navigation: any }) {
  ...
  const handlePostTeam = () => {
    if (!teamName || availability <= 0) {
      Alert.alert(
        "Error",
        "Por favor completa todos los campos obligatorios y asegúrate de que la disponibilidad sea mayor a 0."
      );
      return;
    }

    setIsPublishing(true); // Cambia el estado para activar el color del botón
    setTimeout(() => {
      Alert.alert("Éxito", "El equipo ha sido publicado correctamente.", [
        {
          text: "OK",
          onPress: () => {
            setIsPublishing(false); // Restablece el estado después de la publicación
            navigation.navigate("CanchasFeed");
          },
        },
      ]);
    }, 1000);
  ...
  const removeMember = (index: number): void => {
    const updatedMembers: string[] = members.filter(
      (_, i: number) => i !== index
    );
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <Text style={[styles.label, styles.spacing]}>
          ¿Cuántas personas estás buscando? (obligatorio)
        </Text>
        <TextInput
          style={styles.input}
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity
            style={[
              styles.button,
              isPublishing ? styles.publishingButton : styles.publishButton,
            ]}
