...
import { useQuery as useQuerySupabase, useInsertMutation, useDeleteMutation, useUpdateMutation } from "@supabase-cache-helpers/postgrest-react-query";
...
export function useUpdateReservation(supabase: SupabaseClient<Database>) {
	...
	return {
		...updateMutation,
		mutateAsync: async ({ id, data }: { id: string, data: Partial<Database["public"]["Tables"]["reservations"]["Update"]> }) => {
			return updateMutation.mutateAsync({
				id,
				...data
			});
		},
		mutate: ({ id, data }: { id: string, data: Partial<Database["public"]["Tables"]["reservations"]["Update"]> }, options?: any) => {
