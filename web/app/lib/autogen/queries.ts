...
export function getAllFields(supabase: SupabaseClient<Database>, opts: any = {}) {
	return useQuerySupabase(supabase.from("fields").select("*"), opts);
}

export function getAllFieldsByOwner(supabase: SupabaseClient<Database>, ownerId: string, opts: any = {}) {
	return useQuerySupabase(supabase.from("fields").select("*").eq("owner", ownerId), opts);
}

export function getFieldById(supabase: SupabaseClient<Database>, fieldId: string, opts: any = {}) {
	return useQuerySupabase(supabase.from("fields").select("*").eq("id", fieldId).single(), opts);
}

export function getAllSports(supabase: SupabaseClient<Database>, opts: any = {}) {
...
export function getAllReservationsForFieldById(supabase: SupabaseClient<Database>, fieldId: string, opts: any = {}) {
	return useQuerySupabase(supabase.from("reservations").select("*").eq("field_id", fieldId), opts);
}

export function getAllReservationTimeSlots(supabase: SupabaseClient<Database>, fieldId: string, opts: any = {}) {
	return useQuerySupabase(supabase.from("reservations").select("date_time").eq("field_id", fieldId), opts);
	// .gte("start_time", startTime);
}

export function getAllTeams(supabase: SupabaseClient<Database>, opts: any = {}) {
	return useQuerySupabase(supabase.from("teams").select("team_id, name, sport, description, images, players"), opts);
}

export function getTeamMembers(supabase: SupabaseClient<Database>, teamId: string, opts: any = {}) {
	return useQuerySupabase(supabase.from("teams").select("players").eq("team_id", teamId).single(), opts);
}

export function getAllUsers(supabase: SupabaseClient<Database>, opts: any = {}) {
	return useQuerySupabase(supabase.from("users").select("id, full_name, avatar_url"), opts);
}

export function getUserAvatar(supabase: SupabaseClient<Database>, user_name: string, opts: any = {}) {
	return useQuerySupabase(supabase.from("users").select("avatar_url").eq("full_name", user_name).single(), opts);
}

export function getUserSession(supabase: SupabaseClient<Database>, opts: any = {}) {
	return useQuery({
...
export function getLastUserPayments(supabase: SupabaseClient<Database>, userId: string, opts: any = {}) {
	return useQuery({
