...
export function ProfilePictureAvatar(
	...
	return (
		<Avatar className="h-10 w-10 rounded-full object-cover border-black overflow-hidden border shadow-md">
			<AvatarImage className="h-full w-full object-cover" src={str || undefined} alt="Profile picture" />
			<AvatarFallback className="w-full h-full flex items-center justify-center text-xs font-bold">
				{name
					?.split(" ")
					.slice(0, 2)
					.map((e) => e.charAt(0))
