...
export const fieldAvailabilitiesTable = pgTable(
	"field_availabilities",
	{
		...
		day_of_week: integer().notNull(), // 0=Lunes a 6=Domingo

		// Hora de inicio y fin de atención (formato 24h)
		start_time: varchar({ length: 5 }).notNull(), // Ej: '08:00'
		end_time: varchar({ length: 5 }).notNull(), // Ej: '22:00'
	},
	(table) => [
		pgPolicy("field_availabilities - select authenticated", {
			for: "select",
			using: sql`true`,
			to: authenticatedRole,
			as: "permissive",
		}),
		pgPolicy("field_availabilities - insert authenticated", {
			for: "insert",
			withCheck: sql`true`,
			to: authenticatedRole,
			as: "permissive",
		}),
		pgPolicy("field_availabilities - update authenticated", {
			for: "update",
			withCheck: sql`true`,
			using: sql`true`,
			to: authenticatedRole,
		...
		pgPolicy("field_availabilities - delete authenticated", {
			for: "delete",
			using: sql`true`,
			to: authenticatedRole,
			as: "permissive",
